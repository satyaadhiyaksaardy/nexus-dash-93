// generate an Ubuntu icon component

import React from 'react';

/**
 * Icon props for inline SVG icons.
 * size: sets both width and height when width/height are not provided.
 * color: sets the SVG fill (defaults to currentColor).
 */
...
export const UbuntuIcon: React.FC<IconProps> = ({ size, color, width, height, ...props }) => {
  const w = width ?? size ?? 24;
  const h = height ?? size ?? 24;
  const fill = color ?? (props.fill as string) ?? 'currentColor';
